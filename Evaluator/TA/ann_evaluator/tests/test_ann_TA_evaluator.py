...
from tentacles.Evaluator.TA import AnnTAEvaluator
...
async def evaluator_tester():
    evaluator_tester_instance = TestAnnEvaluator()
    evaluator_tester_instance.TA_evaluator_class = AnnTAEvaluator
    return evaluator_tester_instance


class TestAnnEvaluator(AbstractTATest):
    ...
    async def test_reactions_to_dump(evaluator_tester):
        await evaluator_tester.run_test_reactions_to_dump(-1, 2, 1, 1, 1)

    @staticmethod
    async def test_reactions_to_pump(evaluator_tester):
        await evaluator_tester.run_test_reactions_to_pump(-1, -1, -1, -1, -1, 1, 1)

    @staticmethod
    async def test_reaction_to_rise_after_over_sold(evaluator_tester):
        await evaluator_tester.run_test_reactions_to_rise_after_over_sold(-1, 1, 1, -1, 1)

    @staticmethod
    async def test_reaction_to_over_bought_then_dip(evaluator_tester):
        await evaluator_tester.run_test_reactions_to_over_bought_then_dip(1, -1, -1, 1, 1, -1)
